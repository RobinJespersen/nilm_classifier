...
class AggregatedWfdknnClassifier(ClassifierMixin, BaseEstimator):

    def __init__(self, location_column_index=0, n_neighbors=1, resolution=10):
        self.location_column_index = {}
        self.locations = {}

        self.clf = KNeighborsClassifier(algorithm='auto', n_neighbors=1)
        ...
        """
        :param resolution: int number of samples to take for aggregated classifier
        """
    ...
    def fit(self, X, y):
        ...
        aggregated_X = [[x] for x in numpy.linspace(0, 1, self.resolution)]
        aggregated_y = self._predict(aggregated_X)
        self.clf.fit(aggregated_X, aggregated_y)
